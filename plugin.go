...
// Plugin represents parsed plugin metadata.
type Plugin struct {
...
// extractHeader searches s for a header using a regular expression and
// returns the trimmed captured value.
//
...
// cleanupHeaderComment removes comment terminators (*/ or ?>) and all
// following content on the same line, then trims surrounding whitespace.
//
